...
class Qwen2PoolerRewardModel(Qwen2PreTrainedModel):
    def __init__(self, config):
        ...
        self.pooler_head = nn.ModuleDict(
            {
                "dense1": nn.Linear(hidden_size, hidden_size),
                "dense2": nn.Linear(hidden_size, self.num_labels),
            }
        )
    ...
    def forward(
        ...
        # logits = self.pooler_head(sequence_output)
        pooler_hidden = self.pooler_head["dense1"](sequence_output)
        pooler_hidden = torch.relu(pooler_hidden)  # 添加激活函数
        logits = self.pooler_head["dense2"](pooler_hidden)
...
class CriticWorker(Worker, DistProfilerExtension):
    def __init__(self, config: FSDPCriticConfig):
        ...
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(
                backend=get_nccl_backend(), init_method=os.environ.get("DIST_INIT_METHOD", None), timeout=timedelta(minutes=1800)
            )
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_type = config.model.get("reward_type", "qwen2")
            print(f"[reward build] config: {config} and {reward_type}")
            if reward_type == "qwen3":
                ...
                # reward_module = Qwen2PoolerRewardModel.from_pretrained(
                reward_module = AutoModelForTokenClassification.from_pretrained(
                    pretrained_model_name_or_path=local_path,
                    config=model_config,
                    torch_dtype=torch.bfloat16,
